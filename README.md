...
- `--suggest-emojis` (default) uses deterministic local suggestions only.
- `--suggest-emojis --ai` uses an external provider.

When `--ai` is used, suggestions will be generated by the external provider based on the environment variables, `--ai-provider`, and `--ai-model`.
